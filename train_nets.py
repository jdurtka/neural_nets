...
#	else:
	...
	logging.info("Building RBF networks...")
	#produce networks for each possible number of RBFs
	for nrbf_basis in [2,3,4]:
	
		#first, generate the means using a grid-sampling approach
		dims = Xs[0].shape[0]
		means = []
		
		#choose a "healthier" amount depending on how high our dimension will be
		if dims < 3:
			nrbf_basis = nrbf_basis * 3
		
		#very hackish, but simplifies things
		for u in range(-nrbf_basis, nrbf_basis):
			for v in range(-nrbf_basis, nrbf_basis):
				u_ = (u/nrbf_basis)*1.5
				v_ = (v/nrbf_basis)*1.5
				if dims == 2:
					means.append([u_, v_])
				else:
					for w in range(-nrbf_basis, nrbf_basis):
						w_ = (w/nrbf_basis)*1.5
						if dims == 3:
							means.append([u_, v_, w_])
						elif dims == 4:
							means.append([u_, v_, w_, 0])
						elif dims == 5:
							means.append([u_, v_, w_, 0, 0])
						else:
							means.append([u_, v_, w_, 0, 0, 0])
		
		logging.info("Finding d_max...")
		#Get the max distance for the sigma heuristic
		#maxes = findMaxes(X=Xs, k=len(means), metric=l2_norm, debug=False)
		#logging.info("d_max found: " + str(maxes[0]))
		
		#BETTER WAY: we're in a hypercube, so the max distance will just be opposite points on the cube!
		#Since we're searching the space [-1.5,1.5], this amounts to a distance of 3 along each dimension
		#However, we are only building RBFs up to 3 dimensions
		dim_d = 0
		if dims < 4:
			ds_dims = dims
		else:
			ds_dims = 3
		for i in range(0,ds_dims):
			dim_d += 3*3
		maxes = np.ones(len(means))*np.sqrt(dim_d)
		
		#next, build an RBF based on the computed k means
		thisrbf = NetInstance(net=RBFNet(num_inputs=len(Xs[0]), num_RBFs=len(means), num_outputs=1, debug=0, seed=4, weightScale=0.5), name='rbf_'+str(len(means))+'/')
		
		thisrbf.net.addRBFs(means, maxes/np.sqrt(2*len(means)))
		
		logging.info("Next RBFnet...")
		
		rbfnets.append(thisrbf)
		
		fulldir=outputDirectory+thisrbf.name
		try:
			os.stat(fulldir)
		except:
			os.mkdir(fulldir)

		f = open(outputDirectory+thisrbf.name+'centroids.txt', 'w')
		f.write(str(means))
		f.write('\n')
		f.write(str(maxes/np.sqrt(2*len(means))))
		f.close()
		
	for netinst in rbfnets:
		relstart = time.time()
		...
		netinst.net.xval_train(Xs, ys, eta=0.03, alpha=0.008, epochs=int(int(args.epochs)/KFOLDS), debug=1, saveStep=int(args.epochs)/10, saveDir=fulldir, savePrefix='weights_', logging=logging, folds=KFOLDS)
		...
		logging.info("RBF training time elapsed: " + str(time.time()-relstart))
